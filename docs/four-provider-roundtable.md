...
The checked-in matrix is `tests/fixtures/four-provider-models.json`. Model substitutions are visible data, not silent aliases. The live workflow refuses to run until an operator explicitly acknowledges both substitutions.
...
`Live four-provider roundtable` is a manual workflow protected by the `live-provider-smoke` GitHub environment. Configure reviewers and these environment secrets:
...
The adapter only permits the fixed official HTTPS destinations encoded in `scripts/agent_pontifex_roundtable/providers.py`, rejects redirects, bounds provider response bytes and text, uses one request per agent per phase, and never passes provider keys to the Rust bridge. A live pass requires eight successful provider calls and the same replay/SSE/pairwise assertions as the credential-free lane.
...
python3 scripts/four_provider_roundtable.py \
  --mode mock \
  --bridge-bearer local-only

kill "$bridge_pid"
```

GitHub and Linear remain the durable human-visible ledgers. The live room transports externally observable coordination; it does not confer tool authority or permission for irreversible writes.
